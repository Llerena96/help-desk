...
export function Sidebar({ isSidebarOpen, setSidebarOpen }) {
  ...
  return (
    <>
      ...
      <button
        className="fixed top-4 left-4 z-[10000] p-2 bg-gray-700 text-white rounded-md"
      ...
      <aside
        className={`fixed top-0 left-0 h-full ${isSidebarOpen ? "w-64" : "w-20"
...
<div className="flex items-center justify-center mb-6">
  <h1
    className={`text-center text-xl font-bold transition-all ${isSidebarOpen ? "opacity-100" : "opacity-0 hidden"} ml-4`}
  >
    Mesa de Ayuda
  </h1>
</div>
        ...
        <nav className="flex flex-col space-y-2 mt-5">
          <SidebarItem
            to="/"
            icon={<Home className="dark:text-white w-5 h-5" />}
            label="Inicio"
